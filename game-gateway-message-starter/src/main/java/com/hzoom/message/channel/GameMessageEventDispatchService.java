...
public class GameMessageEventDispatchService {
    ...
    public GameMessageEventDispatchService(ApplicationContext context, GameEventExecutorGroup workerGroup, IMessageSendFactory messageSendFactory, RpcMessageSendFactory rpcMessageSendFactory, GameChannelInitializer channelInitializer) {
    ...
    private GameChannel getGameChannel(Long playerId) {
        ...
        if (gameChannel == null) {// 从集合中获取一个GameChannel，如果这个GameChannel为空，则重新创建，并初始化注册这个Channel，完成GameChannel的初始化。
            gameChannel = new GameChannel(playerId, this, messageSendFactory, rpcMessageSendFactory);
    ...
    public void fireReadMessage(Long playerId, IMessage message) {// 发送接收到的消息事件
        this.safeExecute(() -> {
    ...
    public void fireReadRPCRequest(IMessage gameMessage) {
        this.safeExecute(() -> {
    ...
    public void broadcastMessage(IMessage gameMessage, long... playerIds) {// 发送消息广播事件，客多个客户端发送消息。
