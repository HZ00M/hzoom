...
import com.hzoom.game.channel.GameChannelOutboundHandler;
...
public class GameChannelIdleStateHandler implements GameChannelInboundHandler, GameChannelOutboundHandler {
    ...
    private byte state; // 0 - none, 1 - initialized, 2 - destroyed   要不然会有这样的情况，虽然GameChannel已被移除，但是当定时事件执行时，又会创建一个新的定时事件，导致这个对象不会被回收
    ...
    public void channelRead(AbstractGameChannelHandlerContext ctx, Object msg) throws Exception {
        if (readerIdleTimeNanos > 0 || allIdleTimeNanos > 0) {
            this.lastReadTime = ticksInNanos();// 记录最后一次读取操作的时间
        }
    ...
    @Override
    public void writeAndFlush(AbstractGameChannelHandlerContext ctx, IMessage msg, GameChannelPromise promise) throws Exception {
    ...
    private IdleStateEvent newIdleStateEvent(IdleState state) {// 获取空闲事件类型
        switch (state) {
            case ALL_IDLE:
